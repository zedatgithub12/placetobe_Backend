<?php
include 'connection.php';
$encodedData = file_get_contents('php://input');
$decodedData = json_decode($encodedData, true);

//$userId = 98;
$userId = $decodedData['userId'];

$followerSQL = "SELECT followed_id FROM follow WHERE follower_id = '$userId'";
$followerQuery= mysqli_query($conn, $followerSQL);
$followerRow = mysqli_num_rows($followerQuery);

if($followerRow>0){
  while($followerArray = mysqli_fetch_assoc($followerQuery)){

        $following = implode("&&",$followerArray);
    //echo json_encode($followers);


  $featchCommand = "SELECT * FROM users WHERE userId= '$following'";
  $featchQuery = mysqli_query($conn, $featchCommand);
  $featchedRow = mysqli_num_rows($featchQuery);

  if($featchedRow > 0){
     $FollowingArray[] = mysqli_fetch_assoc($featchQuery);
     $TotalFollowing  = $FollowingArray;
     $message = "succeed";
  }
else {
  $TotalFollowing  = null;
  $message = "not featched";
}
}
}
else {
  $TotalFollowing  = null;
  $message = "no follower yet";
}

$response[] = array("message"=>$message, "following"=>$TotalFollowing);
echo json_encode($response);
mysqli_close($conn);


 ?>
